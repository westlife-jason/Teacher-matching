...
function getTransport(distanceKm: number): {
  mode: TransportMode;
  label: string;
  minutesMin: number;
  minutesMax: number;
  ...
  if (distanceKm < 1.0) {
    const m = Math.max(3, Math.round(distanceKm * 13));
    return { mode: "walk", label: "도보", minutesMin: m, minutesMax: m + 2, bg: "bg-emerald-50", text: "text-emerald-700", border: "border-emerald-200" };
  } else if (distanceKm < 5.0) {
    const m = Math.round(distanceKm * 3 + 5);
    return { mode: "bus", label: "버스", minutesMin: m, minutesMax: m + 5, bg: "bg-blue-50", text: "text-blue-700", border: "border-blue-200" };
  } else {
    const m = Math.round(distanceKm * 1.8 + 8);
    return { mode: "subway", label: "지하철", minutesMin: m, minutesMax: m + 5, bg: "bg-violet-50", text: "text-violet-700", border: "border-violet-200" };
  }
}
        ...
        {result && !loading && (
          <div className="space-y-3">
            {result.centers.map((center, idx) => {
              const t = getTransport(center.distanceKm);
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="mt-2 h-1.5 rounded-full bg-gray-100 overflow-hidden">
                        <div
                          className={`h-full rounded-full ${BAR_COLOR[idx]}`}
                      ...
                      <div className="mt-2 flex items-center gap-2">
                        <span className={`inline-flex items-center gap-1.5 text-xs font-medium px-2.5 py-1 rounded-full border ${t.bg} ${t.text} ${t.border}`}>
            ...
            <div className="flex gap-3 pt-1 justify-center flex-wrap">
              {[
                ...
                { mode: "bus"  as TransportMode, label: "버스 (1~5km)", bg: "bg-blue-50",   text: "text-blue-700",   border: "border-blue-200" },
                { mode: "subway" as TransportMode, label: "지하철 (5km+)", bg: "bg-violet-50", text: "text-violet-700", border: "border-violet-200" },
              ].map((item) => (
